    ...
    const Slider::SliderStyle, Slider&)
{
    ...
    g.fillRect(Rectangle<float>(x, y + height/2 - barHeight/2, width, barHeight));

    // thumb
    int thumbWidth = width * 0.075;
    g.fillRect(Rectangle<float>((int)sliderPos - (thumbWidth /2), y, thumbWidth, height));
    ...
    g.fillRect(Rectangle<float>((int)sliderPos - (detailWidth / 2), y + (height - detailHeight)/2, detailWidth, detailHeight));
}